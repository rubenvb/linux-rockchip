...
struct rockchip_vpu_h264_dec_priv_tbl {
	...
	if (slice->flags & V4L2_H264_SLICE_FLAG_FIELD_PIC) {
		for (i = 0; i < VPU_H264_NUM_DPB * 2; ++i) {
			...
			if (dpb[i / 2].flags & V4L2_H264_DPB_ENTRY_FLAG_LONG_TERM)
				dpb_longterm |= BIT(VPU_H264_NUM_DPB * 2 - 1 - i);
		}
		...
		for (i = 0; i < VPU_H264_NUM_DPB; ++i) {
			if (dpb[i].flags & V4L2_H264_DPB_ENTRY_FLAG_ACTIVE)
				dpb_valid |= BIT(VPU_H264_NUM_DPB - 1 - i);

			if (dpb[i].flags & V4L2_H264_DPB_ENTRY_FLAG_LONG_TERM)
				dpb_longterm |= BIT(VPU_H264_NUM_DPB - 1 - i);
		}
...
int rockchip_vpu_h264_dec_init(struct rockchip_vpu_ctx *ctx)
{
	...
	struct rockchip_vpu_h264_dec_priv_tbl *tbl;

	priv->size = sizeof(struct rockchip_vpu_h264_dec_priv_tbl);
	priv->cpu = dma_alloc_coherent(vpu->dev, priv->size, &priv->dma,
				       GFP_KERNEL);
	if (!priv->cpu)
		return -ENOMEM;
