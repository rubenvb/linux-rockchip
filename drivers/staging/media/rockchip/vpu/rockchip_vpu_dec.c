			   ...
			   struct v4l2_capability *cap)
{
	...
	strscpy(cap->card, vpu->vfd_dec->name, sizeof(cap->card));
				    ...
				    struct rockchip_vpu_ctx *ctx)
{
	...
	fill_pixfmt_mp(fmt, ctx->vpu_dst_fmt->fourcc, width, height);
}
...
static int rockchip_vpu_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	/* Set codec_ops for the chosen source format */
	codec_mode = ctx->vpu_src_fmt->codec_mode;

	vpu_debug(4, "Codec mode = %d\n", codec_mode);
	ctx->codec_ops = &ctx->dev->variant->codec_ops[codec_mode];

	if (V4L2_TYPE_IS_OUTPUT(q->type) && ctx->codec_ops->start)
		ctx->codec_ops->start(ctx);
...
static void rockchip_vpu_stop_streaming(struct vb2_queue *q)
{
	...
	if (V4L2_TYPE_IS_OUTPUT(q->type) && ctx->codec_ops->stop)
