...
static void *rockchip_vpu_get_ctrl(struct rockchip_vpu_ctx *ctx, u32 id)
{
...
static dma_addr_t rockchip_vpu_get_ref(struct vb2_queue *q, u64 ts)
{
	int index;
...
void rk3288_vpu_mpeg2_dec_run(struct rockchip_vpu_ctx *ctx)
{
	...
	dma_addr_t addr, current_addr, forward_addr, backward_addr;
	u32 reg;
...
#define VDPU_REG_APF_THRESHOLD(v)	(((v) << 0) & GENMASK(13, 0))
	...
	case V4L2_MPEG2_PICTURE_CODING_TYPE_P:
		...
		/* Fall back to forward ref frame */
		if (!backward_addr)
			backward_addr = forward_addr;
	}
	...
	vdpu_write_relaxed(vpu, reg, VDPU_SWREG(1));
}
